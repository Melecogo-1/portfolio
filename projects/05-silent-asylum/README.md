...
> **怎么看**：`templates/index.html` 是 Flask 的 Jinja **服务端模板，不能直接当静态网页打开**（否则会看到未渲染的模板标记）；请按下方步骤 `python app.py` 后访问 http://localhost:8080 由 Flask 渲染。游戏内容全部在 `game_data.json`，前端逻辑在 `static/`。
