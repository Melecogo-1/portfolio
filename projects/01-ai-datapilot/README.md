...
> **怎么看**：在线直接打开 [`case-study/index.html`](case-study/index.html) 是完整业务分析案例（静态页、无需后端）；可交互经营看板按下方步骤本地启动（端口 5000）。仓内 `data/processed/` 顶部营收 / 销量来自真实聚合表，订单明细为结构一致的脱敏采样（见 `data/DATA_NOTE.md`，`tools/generate_sample_orders.py` 可复现）。
...
- 自然语言查数采用**受限生成而非 NL2SQL**：把预聚合指标摘要注入提示词、用指标白名单约束模型，只在给定聚合结果范围内作答并标注来源与置信度；未配置 DeepSeek Key 时自动回退到关键词规则，两条路径返回结构一致。
