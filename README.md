...
- **只看网页**：上方在线 3D 作品集；01 的完整业务分析案例可直接打开 [case-study](projects/01-ai-datapilot/case-study/index.html)，纯静态、无需后端。
- **看可交互系统**：五个项目源码都在 `projects/`，每个目录有 README 与启动命令，本地按步骤起服务即可。02–05 的交互依赖各自后端；其中 05 的 `templates/` 是 Flask 服务端模板，需要经 Flask 渲染，不要直接当静态网页打开。
- **数据合规**：01 只公开聚合结果与一份脱敏采样（2800 行、无真实客户信息），企业经营明细不入仓，说明见 [DATA_NOTE.md](projects/01-ai-datapilot/data/DATA_NOTE.md)。
...
**01 AI-DataPilot**：用 pandas 统一字段、对姓名与手机号脱敏，聚合 4.9 万报价单 / 15.6 万账单（20.15 亿营收、28.4 万台）；自然语言查数走「预聚合指标摘要注入 + 指标白名单」做受限生成（非 NL2SQL），模型只在给定指标范围内作答并标注来源与置信度，无 Key 时回退到关键词规则。

**02 Dream Weaver**：约 1250 行后端承载 4 地点 / 4 NPC / 17 因果卡 / 27 事件卡 / 5 结局；用 9 种谓词写声明式 DSL（支持 AND/OR 嵌套），按特异度与 priority 排序裁决；模型单步态度增量钳在 [-8, 8]，规则判不了才回退模板，两条降级路径相互独立。
